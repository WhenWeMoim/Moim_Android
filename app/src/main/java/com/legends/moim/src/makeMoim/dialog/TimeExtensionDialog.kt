...
@SuppressLint("NewApi")
class TimeExtensionDialog(context : Context) {
...
//    private lateinit var tvMessage: TextView
    ...
    interface TimeDialogClickListener {
        fun onExtensionTimeDialogOKClicked( startTimeHour: Int, endTimeHour: Int )
//        fun onCancelClicked()
    }

    fun showExtensionTimeDialog() {
        dialog.window?.setBackgroundDrawableResource(android.R.color.transparent)
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE)
        dialog.setContentView(R.layout.dialog_time_extension)
        dialog.setCancelable(false)

        startTimePicker = dialog.findViewById(R.id.dialog_time_ex_start_tp)
        endTimePicker = dialog.findViewById(R.id.dialog_time_ex_end_tp)

        initTimePicker()

        btnOK = dialog.findViewById(R.id.dialog_time_ex_ok_btn_tv)
        btnOK.setOnClickListener {
            val startTimeHour = startTimePicker.hour
            val endTimeHour = endTimePicker.hour

            listener!!.onExtensionTimeDialogOKClicked(startTimeHour, endTimeHour)
